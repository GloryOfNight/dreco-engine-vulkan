...
void de::vulkan::scene::create(const de::gltf::model& m)
{
	...
	{
		recurseSceneNodes(m, m._nodes[nodeIndex], de::math::mat4::makeIdentity(), info);
	}
...
void de::vulkan::scene::recurseSceneNodes(const de::gltf::model& m, const de::gltf::node& selfNode, const de::math::mat4& rootMat, scene_meshes_info& info)
{
	const auto newRootMat = selfNode._matrix * rootMat;
	if (selfNode._mesh != UINT32_MAX)
	{
		...
		for (const auto& primitive : mesh._primitives)
		{
			...
			auto& newMesh = meshes.emplace_back(new de::vulkan::mesh());
